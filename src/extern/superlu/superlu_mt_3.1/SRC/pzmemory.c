...
doublecomplex *doublecomplexMalloc(int_t n)
{
    ...
    if ( !buf ) {
        superlu_abort_and_exit("SUPERLU_MALLOC failed for buf in doublecomplexMalloc()");
    }
    return (buf);
}
...
doublecomplex *doublecomplexCalloc(int_t n)
{
    ...
    buf = (doublecomplex *) SUPERLU_MALLOC( (size_t) n * sizeof(doublecomplex) );
    if ( !buf ) {
        superlu_abort_and_exit("SUPERLU_MALLOC failed for buf in doublecomplexCalloc()");
    }
