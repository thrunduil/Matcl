 ...
 */
  ...
  MPI_Alltoall (nnzToSend, 1, mpi_int_t, nnzToRecv, 1, mpi_int_t, (*num_comm));
  ...
  ptr_toSnd = intCalloc_symbfact((int_t) nprocs_num);
  ...
  {
    /*
    //this crashes visual studio; replaces by two unconditional assignments
    if (p != iam) 
      ptr_toSnd[p] = i;
    else
      ptr_toSnd[p] = j;
