...
#include "matcl-matrep/func/bin/op_mod.h"
...
matcl::Matrix matcl::rem(const Matrix& A0, const Matrix& B0)
{
    Matrix A(A0);
    Matrix B(B0);

    matcl::Matrix ret;
    details::eval_rem::make(A,B,ret);
    return ret;
};

matcl::Matrix matcl::rem(Matrix&& A0, const Matrix& B0)
{
...
matcl::Matrix matcl::rem(Matrix&& A0, Matrix&& B0)
{
    Matrix A(std::move(A0));
...
matcl::Matrix matcl::rem(const Matrix& A0, Matrix&& B0)
{
    Matrix A(A0);
    Matrix B(std::move(B0));
