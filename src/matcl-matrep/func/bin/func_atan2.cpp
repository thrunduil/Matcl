...
#include "matcl-matrep/func/raw/bin/raw_func_atan2.h"
...
class op_atan2 : public op_no_cases<op_atan2>
{
    public:
        ...
        { 
            ...
            ti_type ti      = ti::get_return_ti<ti_type>(mdyf::atan2::eval(),t1,t2);
        ...
        {
            return mrd::atan2_helper_mat_mat_inpl<M1,M2>::eval(ret, mat1, mat2);
        };

        template<class M1, class S2>
        static void eval_mat_scal(matcl::Matrix& ret, const M1& mat1, const S2& scal)
        {
            return mrd::atan2_helper_mat_scal_inpl<M1,S2>::eval(ret, mat1, scal);
        };

        template<class S1, class M2>
        static void eval_scal_mat(matcl::Matrix& ret, const S1& scal, const M2& mat)
        {
            return mrd::atan2_helper_scal_mat_inpl<S1,M2>::eval(ret, scal, mat);
        };

        template<class S1, class S2>
        static void eval_scal_scal(matcl::Matrix& ret, const S1& scal1, const S2& scal2)
        {
            ret = mrd::atan2_helper<S1,S2>::eval(scal1,scal2);
        };
};

struct eval_atan2 : public extract_type2_switch<void,eval_atan2,mr::val_type_corrector_diag_dense>
{
    template<class T1, class T2>
    static void eval_mat_mat(const T1& A, const T2& B, matcl::Matrix& ret)
    {
        op_atan2 op;
        return md::function_op<T1,T2,op_atan2,false>::eval(ret,A,B,op);
    };

    template<class T1, class T2>
    static void eval_scal_scal(const T1& A, const T2& B, matcl::Matrix& ret)
    {
        ret = mrd::atan2_helper<T1,T2>::eval(A,B);
    };
...
matcl::Matrix matcl::atan2(const Matrix &A0, const Matrix &B0)
{
    Matrix A(A0);
    Matrix B(B0);

    matcl::Matrix ret;
    details::eval_atan2::make(A,B,ret);
    return ret;
};

matcl::Matrix matcl::atan2(Matrix&& A0, const Matrix& B0)
{
...
matcl::Matrix matcl::atan2(Matrix&& A0, Matrix&& B0)
{
    Matrix A(std::move(A0));
...
matcl::Matrix matcl::atan2(const Matrix& A0, Matrix&& B0)
{
    Matrix A(A0);
    Matrix B(std::move(B0));
