...
#include "matcl-matrep/func/raw/bin/raw_func_idiv.h"
...
class idiv_functor
{
    ...
    public:
        idiv_functor(const arg_type& arg) : arg(arg) {};
        ...
        struct eval_impl
        {
            ...
            {
                return idiv_helper<val_type,arg_type>::eval(val,arg);
            };
        };
        ...
        struct eval_impl<ret_type,val_type,true>
        {
            static ret_type eval(const val_type& val, const arg_type& arg)
            {
                return idiv_helper<arg_type,val_type>::eval(arg,val);
            };
        };
};

template<bool rev>
class idiv_functor<Object,rev>
{
    private:
...
struct eval_idiv_functor
{
    ...
    {
        return idiv_helper<T1,T2>::eval(arg1,arg2);
    };
    ...
    static ret eval_zero(ti::ti_type<ret> ti_ret, const T1& arg1, ti::ti_type<TZ> ti_z)
    {
        ...
        return zero_on_right? idiv_helper<T1,TZ>::eval(arg1,zero) 
                            : idiv_helper<TZ,T1>::eval(zero,arg1);
    };
    ...
    static ti::ti_type<ret> return_type(TI1 t1, TI2 t2)
    {
        if (md::runtime_value<bool, is_inv>::eval() == false)
            return ti::get_return_ti<ti::ti_type<ret>>(mdyf::idiv::eval(),t1,t2);
        else
            return ti::get_return_ti<ti::ti_type<ret>>(mdyf::idiv::eval(),t2,t1);
    };

    template<class T, bool is_rev>
    static idiv_functor<T,is_rev> get_scalar_functor(const T& val)
    {
...
void idiv_helper_mat_mat_inpl<M1,M2>::eval(matcl::Matrix& ret, const M1& A, const M2& B)
{
    return eval_op_impl<ret_type_idiv,M1,M2,eval_idiv_functor>::eval(ret,A,B);
};

template<class M1,class M2>
void idiv_helper_scal_mat_inpl<M1,M2>::eval(matcl::Matrix& ret, const M1& A, const M2& B)
{
    return eval_scalar_impl<ret_type_idiv,M2,eval_idiv_functor,false>::eval(ret,B,true,A);
};

template<class M2,class M1>
void idiv_helper_mat_scal_inpl<M2,M1>::eval(matcl::Matrix& ret, const M2& B, const M1& A)
{
    return eval_scalar_impl<ret_type_idiv,M2,eval_idiv_functor,false>::eval(ret,B,false,A);
};

}}}

MACRO_INSTANTIATE_SG_2(matcl::raw::details::idiv_helper_scal_mat_inpl)
MACRO_INSTANTIATE_SST_2(matcl::raw::details::idiv_helper_scal_mat_inpl)

MACRO_INSTANTIATE_GS_2(matcl::raw::details::idiv_helper_mat_scal_inpl)
MACRO_INSTANTIATE_STS_2(matcl::raw::details::idiv_helper_mat_scal_inpl)

MACRO_INSTANTIATE_BIN_ALL(matcl::raw::details::idiv_helper_mat_mat_inpl)
