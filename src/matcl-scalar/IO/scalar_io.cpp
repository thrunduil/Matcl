...
namespace matcl { namespace raw { namespace details
{
    ...
    struct obj_handle : md::object_disp_handle
    {
        const Object* ptr;

        obj_handle(const Object& obj)
            :ptr(&obj)
        {};
        ...
        void disp_elem(md::printer& p, Integer w, align_type at, 
                                    Integer value_pos) const override
        {
            return matcl::disp_object(*ptr, p, w, at, value_pos);
        };
    };

    template<>
    struct elem_handle_type<Object>
    {
        using type = obj_handle;
    };
    ...
    struct disp_matrix<Object,struct_scalar>
    {
        static void eval(md::disp_stream_impl& os, const disp_stream* user, const Object& i)
        {
            ...
            switch(dm)
            {
                ...
                {
                    os.do_disp_new_line(user,1);
                    os.get_printer().disp_elem(-os.do_get_max_matrix_width(user), oh,
                                               align_type::left, 0);
                    os.do_disp_end_line(user,1);
