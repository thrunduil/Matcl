...
bool compute_block(i_type& BS, i_type BB, i_type n, i_type K, i_type max_eig_in_window, i_type max_window_size,
              i_type* sel_vec, V* t, i_type ldt, V* Q, i_type LDQ, 
        ...
        while (WS != BS)
        {
            ...
            while (!calc_T1 || !calc_T2 || !calc_Q)
            {
                ...
                else if (calc_T1 == false)
                {
                    ...
                    if (tl.is_granted() == true)
                    {
                        ...
                        gemm<V>("T", "N", wsize, n - WS + 1 - wsize, wsize, 1.0, work_q, wsize, 
                                work_mat, wsize, 0.0, t + WS - 1 + (WS - 1 + wsize) * ldt, ldt);
                ...
                else if (calc_T2 == false)
                {
                    ...
                    if (tl.is_granted() == true)
                    {
                        lacpy("N", WS - 1, wsize, t + (WS - 1) * ldt, ldt, work_mat, WS - 1);
        ...
        typename details::complex_type<V>::type *w, i_type* m, V* work, i_type lwork, i_type* info)
{
    ...
    i_type M            = analyse_select<V>::eval(n, select, t, ldt, sel_vec.data());
    ...
    if ( M == n || M == 0 )
    {
        compute_eigenvalues<V>::eval(n, t, ldt, w);
        return;
    };
    ...
    auto comp_f         = [n,K,max_eig_in_window,max_window_size, &sel_vec,t,ldt,Q,LDQ,
                        ...
                        { 
                            return compute_block<V>(BS, BB, n, K, max_eig_in_window, max_window_size, 
                                    sel_vec.data(), t, ldt, Q, LDQ, wantq, &t_lock, work, lwork);
                        };
