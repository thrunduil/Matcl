...
lapack::gs_ort(const char* TRANS, i_type N, i_type K, const V* Q, i_type LDQ, V* X, i_type INCX, 
        V* s, i_type INCS, typename details::real_type<V>::type& x_norm, V* work, i_type& INFO)
{
    ...
    VR x_norm0              = lapack::nrm2(N, X, INCX_pos);
    ...
    {
        lapack::gemv(tr_char_1, NV, KV, V(1.0), Q, LDQ, X, INCX, V(0.0), work, 1);
...
gs_ort<d_type>(const char* TR, i_type N, i_type K, const d_type* Q, i_type LDQ, d_type* X, 
        i_type incx, d_type* s, i_type incs, d_type& x_norm, d_type* work, i_type& info);
template BLAS_EXT_EXPORT 
void gs_ort<s_type>(const char* TR, i_type N, i_type K, const s_type* Q, i_type LDQ, s_type* X, 
       i_type incx, s_type* s, i_type incs, s_type& x_norm, s_type* work, i_type& info);
template BLAS_EXT_EXPORT
void gs_ort<z_type>(const char* TR, i_type N, i_type K, const z_type* Q, i_type LDQ, z_type* X, 
        i_type incx, z_type* s, i_type incs, d_type& x_norm, z_type* work, i_type& info);
template BLAS_EXT_EXPORT
void gs_ort<c_type>(const char* TR, i_type N, i_type K, const c_type* Q, i_type LDQ, c_type* X, 
        i_type incx, c_type* s, i_type incs, s_type& x_norm, c_type* work, i_type& info);
