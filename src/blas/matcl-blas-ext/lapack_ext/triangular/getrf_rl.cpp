    ...
    else
    {
        ...
        for(J = 1; J <= lapack::minimum( M, N ); J += NB)
        {
            ...
            if( J+JB <= N )
            {
                ...
                lapack::trsm( "Left", "Lower", "No transpose", "Unit", JB,
                       N-J-JB+1, ONE, A+J+(J-1)*LDA, LDA, A+J+(J+JB-1)*LDA, LDA );
...
getrf_rl<d_type>(i_type M, i_type N, d_type *A, i_type LDA, i_type* IPIV, i_type *INFO );
template void BLAS_EXT_EXPORT
getrf_rl<z_type>(i_type M, i_type N, z_type *A, i_type LDA, i_type* IPIV, i_type *INFO );
