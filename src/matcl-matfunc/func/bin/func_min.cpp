...
class op_min : public op_no_cases<op_min>
{
    public:
        ...
        {
            return mrd::min_helper_mat_mat_inpl<M1,M2>::eval(ret, mat1, mat2);
        };
        ...
        static void eval_mat_scal(matcl::Matrix& ret, const M1& mat, const S2& scal)
        {
            if (mrd::is_geq_zero(scal))
	            return mrd::min_helper_mat_scal_inpl<M1,S2>::eval2(ret,mat,scal);
            else
	            return mrd::min_helper_mat_scal_inpl<M1,S2>::eval(ret,mat,scal);
        };
        ...
        {
            if (mrd::is_geq_zero(scal))
	            return mrd::min_helper_scal_mat_inpl<S1,M2>::eval2(ret,scal,mat);
            else
	            return mrd::min_helper_scal_mat_inpl<S1,M2>::eval(ret,scal,mat);
        };

        template<class S1, class S2>
        static void eval_scal_scal(matcl::Matrix& ret, const S1& scal1, const S2& scal2)
        {
            ret = mrd::min_helper<S1,S2>::eval(scal1,scal2);
        };
};

struct eval_min : public extract_type2_switch<void,eval_min,mr::val_type_corrector_diag>
{
    template<class T1, class T2>
    static void eval_mat_mat(const T1& A, const T2& B, matcl::Matrix& ret)
    {
        op_min op;
        return md::function_op<T1,T2,op_min,false>::eval(ret,A,B,op);
    };

    template<class T1, class T2>
    static void eval_scal_scal(const T1& A, const T2& B, matcl::Matrix& ret)
    {
        ret = mrd::min_helper<T1,T2>::eval(A,B);
    };
...
matcl::Matrix matcl::min(const Matrix& A0, const Matrix& B0)
{
    Matrix A(A0);
    Matrix B(B0);

    matcl::Matrix ret;
    details::eval_min::make(A,B,ret);
    return ret;
};

matcl::Matrix matcl::min(Matrix&& A0, const Matrix& B0)
{
...
matcl::Matrix matcl::min(Matrix&& A0, Matrix&& B0)
{
    Matrix A(std::move(A0));
...
matcl::Matrix matcl::min(const Matrix& A0, Matrix&& B0)
{
    Matrix A(A0);
    Matrix B(std::move(B0));
