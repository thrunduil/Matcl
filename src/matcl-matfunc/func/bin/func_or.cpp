...
#include "matcl-matfunc/func/raw/bin/raw_func_or.h"
...
class op_or_h : public op_info<op_or_h>
{
    public:
        template<class Val>
        ...
        { 
            if (mrd::is_zero(val) == false)     ret = one(mat);
        ...
        {
            return mrd::or_helper_mat_mat_inpl<M1,M2>::eval(ret, mat1, mat2);
        };

        template<class M1, class S2>
        static void eval_mat_scal(matcl::Matrix& ret, const M1& mat1, const S2& scal)
        {
            return mrd::or_helper_mat_scal_inpl<M1,S2>::eval(ret, mat1, scal);
        };

        template<class S1, class M2>
        static void eval_scal_mat(matcl::Matrix& ret, const S1& scal, const M2& mat)
        {
            return mrd::or_helper_scal_mat_inpl<S1,M2>::eval(ret, scal, mat);
        };

        template<class S1, class S2>
        static void eval_scal_scal(matcl::Matrix& ret, const S1& scal1, const S2& scal2)
        {
            ret = matcl::Matrix(mrd::elem_or_helper<S1,S2>::eval(scal1,scal2));
        };
};

struct eval_or : public extract_type2_switch<void,eval_or,mr::val_type_corrector_bool>
{
    template<class T1, class T2>
    static void eval_mat_mat(const T1& A, const T2& B, matcl::Matrix& ret)
    {
        op_or_h op;
        return md::function_op<T1,T2,op_or_h,false>::eval(ret,A,B,op);
    };

    template<class T1, class T2>
    static void eval_scal_scal(const T1& A, const T2& B, matcl::Matrix& ret)
    {
        ret = Matrix(mrd::elem_or_helper<T1,T2>::eval(A,B));
    };
...
matcl::Matrix matcl::elem_or(const Matrix& A0, const Matrix& B0)
{
    Matrix A(A0);
    Matrix B(B0);

    matcl::Matrix ret;
    details::eval_or::make(A,B,ret);
    return ret;
};

matcl::Matrix matcl::elem_or(Matrix&& A0, const Matrix& B0)
{
...
matcl::Matrix matcl::elem_or(Matrix&& A0, Matrix&& B0)
{
    Matrix A(std::move(A0));
...
matcl::Matrix matcl::elem_or(const Matrix& A0, Matrix&& B0)
{
    Matrix A(A0);
    Matrix B(std::move(B0));
