...
#include "matcl-matfunc/func/raw/bin/raw_func_rem.h"
...
class rem_functor
{
    ...
    public:
        rem_functor(const arg_type& arg) : arg(arg) {};
                                   ...
                                   const in_type &x) const
        {
            ...
            ret = mrd::create_matrix<val_real>(ret_ti, x.rows(), x.cols());
        };
        ...
        struct eval_impl
        {
            ...
            {
                return rem_helper<val_type,arg_type>::eval(val,arg);
            };
        };
        ...
        struct eval_impl<ret_type,val_type,true>
        {
            static ret_type eval(const val_type& val, const arg_type& arg)
            {
                return rem_helper<arg_type,val_type>::eval(arg,val);
            };
        };
};

template<bool rev>
class rem_functor<Object,rev>
{
    private:
...
struct eval_zero_rem<Object,Object,zero_on_right>
{
    using T = Object;
    static T eval(const T& arg1, ti::ti_type<T> ti_z)
    {
        T zero = md::default_value<T>(ti_z);
        return zero_on_right? rem_helper<T,T>::eval(arg1,zero)
                              :rem_helper<T,T>::eval(zero,arg1);
    }
};
...
struct eval_rem_functor
{
    ...
    {
        return rem_helper<T1,T2>::eval(arg1,arg2);
    };
    ...
    {
        (void)ti_z;
        return eval_zero_rem<ret,T1,zero_on_right>::eval(arg1, ti_z);
    };
    ...
    static ti::ti_type<ret> return_type(TI1 t1, TI2 t2)
    {
        if (md::runtime_value<bool, is_inv>::eval() == false)
            return ti::get_return_ti<ti::ti_type<ret>>(mdyf::rem::eval(),t1,t2);
        else
            return ti::get_return_ti<ti::ti_type<ret>>(mdyf::rem::eval(),t2,t1);
    };

    template<class T, bool is_rev>
    static rem_functor<T,is_rev> get_scalar_functor(const T& val)
    {
...
void rem_helper_mat_mat_inpl<M1,M2>::eval(matcl::Matrix& ret, const M1& A, const M2& B)
{
    return eval_op_impl<ret_type_rem,M1,M2,eval_rem_functor>::eval(ret,A,B);
};

template<class M1,class M2>
void rem_helper_scal_mat_inpl<M1,M2>::eval(matcl::Matrix& ret, const M1& A, const M2& B)
{
    return eval_scalar_impl<ret_type_rem,M2,eval_rem_functor,false>::eval(ret,B,true,A);
};

template<class M2,class M1>
void rem_helper_mat_scal_inpl<M2,M1>::eval(matcl::Matrix& ret, const M2& B, const M1& A)
{
    return eval_scalar_impl<ret_type_rem,M2,eval_rem_functor,false>::eval(ret,B,false,A);
};

}}}

MACRO_INSTANTIATE_SG_DIAG_DENSE_2(matcl::raw::details::rem_helper_scal_mat_inpl)
MACRO_INSTANTIATE_GS_DIAG_DENSE_2(matcl::raw::details::rem_helper_mat_scal_inpl)
MACRO_INSTANTIATE_BIN_DIAG_DENSE(matcl::raw::details::rem_helper_mat_mat_inpl)
