...
class plus_functor
{
    ...
    public:
        plus_functor(const arg_type& arg) : arg(arg) {};
        ...
        {
            return plus_helper<val_type,arg_type>::eval(val,arg);
        };
};

template<bool rev>
class plus_functor<Object,rev>
{
    private:
...
struct eval_zero_plus<Object,Object,zero_on_right>
{
    using T = Object;
    static T eval(const T& arg1, ti::ti_type<T> ti_z)
    {
        T zero = md::default_value<T>(ti_z);
        return zero_on_right? plus_helper<T,T>::eval(arg1,zero)
                              :plus_helper<T,T>::eval(zero,arg1);
    }
};
...
struct eval_plus_functor
{
    ...
    {
        return plus_helper<T1,T2>::eval(arg1,arg2);
    };
    ...
    {
        (void)ti_ret;
        return eval_zero_plus<ret,T1,zero_on_right>::eval(arg1, ti_z);
    };
    ...
    static ti::ti_type<ret> return_type(TI1 t1, TI2 t2)
    {
        if (md::runtime_value<bool, is_inv>::eval() == false)
            return ti::get_return_ti<ti::ti_type<ret>>(mdyf::op_plus::eval(),t1, t2);
        else
            return ti::get_return_ti<ti::ti_type<ret>>(mdyf::op_plus::eval(),t2, t1);
    };

    template<class T, bool is_rev>
    static plus_functor<T,is_rev> get_scalar_functor(const T& val)
    {
    ...
    {
        return predefined_struct_ext::plus_struct(f1,f2, is_re_1, is_re_2, is_square);
    };

    template<bool zero_on_right>
    using is_eval_zero_id   = std::true_type;
};
...
template<class M1,class M2>
void plus_helper_scal_mat_inpl<M1,M2>::eval(matcl::Matrix& ret, const M1& A, const M2& B)
{
    return eval_scalar_impl<ret_type_plus,M2,eval_plus_functor,false>::eval(ret,B,true,A);
};

template<class M2,class M1>
void plus_helper_mat_scal_inpl<M2,M1>::eval(matcl::Matrix& ret, const M2& B, const M1& A)
{
    return eval_scalar_impl<ret_type_plus,M2,eval_plus_functor,false>::eval(ret,B,false,A);
};

}}}

MACRO_INSTANTIATE_SG_2(matcl::raw::details::plus_helper_scal_mat_inpl)
MACRO_INSTANTIATE_SST_2(matcl::raw::details::plus_helper_scal_mat_inpl)

MACRO_INSTANTIATE_GS_2(matcl::raw::details::plus_helper_mat_scal_inpl)
MACRO_INSTANTIATE_STS_2(matcl::raw::details::plus_helper_mat_scal_inpl)

MACRO_INSTANTIATE_BIN_ALL(matcl::raw::details::plus_helper_mat_mat_inpl)
