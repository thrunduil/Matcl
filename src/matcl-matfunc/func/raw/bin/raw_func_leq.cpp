...
#include "matcl-matfunc/func/raw/bin/raw_func_leq.h"
...
class leq_functor
{
    ...
    public:
        leq_functor(const arg_type& arg) : arg(arg) {};
        ...
        struct eval_impl<ret, val_type,true>
        {
            static ret eval(const val_type& val, const arg_type& arg)
            {
                return mrd::leq_helper<arg_type,val_type>::eval(arg,val);
            };
        };
};

template<class Ret, class T, bool zero_on_right>
struct eval_zero_leq
{
    static Ret eval(T arg1, ti::ti_type<T> ti_z)
    {
        (void)ti_z;
        return zero_on_right? mrd::is_leq_zero(arg1) : mrd::is_geq_zero(arg1);
    }
};

template<bool zero_on_right>
struct eval_zero_leq<Object,Object,zero_on_right>
{
    using T = Object;
    static T eval(const T& arg1, ti::ti_type<T> ti_z)
    {
        T zero = md::default_value<T>(ti_z);
        return zero_on_right? leq_helper<T,T>::eval(arg1,zero)
                              :leq_helper<T,T>::eval(zero,arg1);
    }
};
...
struct eval_leq_functor
{
    ...
    {
        return mrd::leq_helper<T1,T2>::eval(arg1,arg2);
    };
    ...
    {
        (void)ti_ret;
        return eval_zero_leq<ret,T1,zero_on_right>::eval(arg1, ti_z);
    };
    ...
    static ti::ti_type<ret> return_type(TI1 t1, TI2 t2)
    {
        if (md::runtime_value<bool, is_inv>::eval() == false)
            return ti::get_return_ti<ti::ti_type<ret>>(mdyf::op_leq::eval(),t1,t2);
        else
            return ti::get_return_ti<ti::ti_type<ret>>(mdyf::op_leq::eval(),t2,t1);
    };

    template<class T, bool is_rev>
    static leq_functor<T,is_rev> get_scalar_functor(const T& val)
    {
...
void leq_helper_mat_mat_inpl<M1,M2>::eval(matcl::Matrix& ret, const M1& A, const M2& B)
{
    return eval_op_impl<ret_type_leq,M1,M2,eval_leq_functor>::eval(ret,A,B);
};

template<class M1,class M2>
void leq_helper_scal_mat_inpl<M1,M2>::eval(matcl::Matrix& ret, const M1& A, const M2& B)
{
    return eval_scalar_impl<ret_type_leq,M2,eval_leq_functor,false>::eval(ret,B,true,A);
};

template<class M2,class M1>
void leq_helper_mat_scal_inpl<M2,M1>::eval(matcl::Matrix& ret, const M2& B, const M1& A)
{
    return eval_scalar_impl<ret_type_leq,M2,eval_leq_functor,false>::eval(ret,B,false,A);
};

template<class M1,class M2>
void leq_helper_scal_mat_inpl<M1,M2>::eval2(matcl::Matrix& ret, const M1& A, const M2& B)
{
    return eval_scalar_impl<ret_type_leq,M2,eval_leq_functor,false>::eval(ret,B,true,A);
};

template<class M2,class M1>
void leq_helper_mat_scal_inpl<M2,M1>::eval2(matcl::Matrix& ret, const M2& B, const M1& A)
{
    return eval_scalar_impl<ret_type_leq,M2,eval_leq_functor,false>::eval(ret,B,false,A);
};
...
MACRO_INSTANTIATE_SG_DIAG_2(matcl::raw::details::leq_helper_scal_mat_inpl)
MACRO_INSTANTIATE_SST_DIAG_2(matcl::raw::details::leq_helper_scal_mat_inpl)

MACRO_INSTANTIATE_GS_DIAG_2(matcl::raw::details::leq_helper_mat_scal_inpl)
MACRO_INSTANTIATE_STS_DIAG_2(matcl::raw::details::leq_helper_mat_scal_inpl)

MACRO_INSTANTIATE_BIN_DIAG(matcl::raw::details::leq_helper_mat_mat_inpl)
