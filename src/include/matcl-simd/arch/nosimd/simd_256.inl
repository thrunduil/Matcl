...
force_inline
simd<double, 256, nosimd_tag>::simd(Integer val)
    : simd(double(val)) 
{}
...
force_inline
simd<double, 256, nosimd_tag>::simd(const impl_type& v)
{
    data[0] = v[0];
    data[1] = v[1];
    data[2] = v[2];
...
template<int Pos>
force_inline
double simd<double, 256, nosimd_tag>::get() const
{ 
    return data[Pos]; 
};
...
force_inline void simd<double, 256, nosimd_tag>::store(double* arr, std::true_type aligned) const
{
    (void)aligned;
    arr[0]  = data[0];
    arr[1]  = data[1];
...
force_inline void simd<double, 256, nosimd_tag>::store(double* arr, std::false_type not_aligned) const
{
    (void)not_aligned;
    arr[0]  = data[0];
    arr[1]  = data[1];
...
force_inline void simd<float, 256, nosimd_tag>::store(float* arr, std::true_type aligned) const
{
    (void)aligned;
    arr[0] = data[0];
    arr[1] = data[1];
...
force_inline void simd<float, 256, nosimd_tag>::store(float* arr, std::false_type not_aligned) const
{
    (void)not_aligned;
    arr[0] = data[0];
    arr[1] = data[1];
