...
class alignas(32) simd<int64_t, 256, avx_tag>
{
    public:
        ...
        // simd type storing float of size 128 bits
        using simd_128_float = simd<float, 128, sse_tag>;

        // simd type storing float  of size 256 bits
        using simd_256_float = simd<float, 256, avx_tag>;

        // simd type storing double values
    ...
    public:
        ...
        // i.e. i-th element of resulting vector is arr[ind[i]]
        static simd     gather(const int64_t* arr, const simd_256_int64& ind);
