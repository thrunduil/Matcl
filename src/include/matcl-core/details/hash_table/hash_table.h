...
class hash_table : private details::ht_base
{
    ...
    private:
        ...
        bool        	        eval_eq_functor(details::check_assign_true, 
                                    const value_type* elem, size_t hash, 
                                    const Key_args& ... key) const;
        
        bool        	        eval_eq_functor(details::check_assign_false,
                                    const value_type* elem, size_t hash, 
                                    const value_type* other) const;

        void                    remove_delete_marks(value_type** pos, 
    ...
    public:
        ...
        hash_table(size_t size = 10, const hasher& hash_func = hasher(), 
                   const equaler& eq_func = equaler());
    ...
    public:
        // internal use only
        const value_type* const*get_entries() const;
};

};

#include "matcl-core/details/hash_table/hash_table.inl"
