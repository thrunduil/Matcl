...
void hash_table<T,H,E,TV,A>::change_size(bool expand)
{
    ...
    for (value_type** entry_ptr = entries; entry_ptr < end_ptr; ++entry_ptr)
    {
        ...
        else if (*entry_ptr > details::mark_delete<value_type*>::value)
        {
            ...
            value_type*& new_entry_ptr	= new_htab->find_entry_impl<false,
                                                const value_type*>(*entry_ptr);
...
bool hash_table<T,H,E,TV,A>::eval_eq_functor(details::check_assign_true, 
                                const value_type* elem,  size_t hash, 
                                const Key_args& ... key) const
{
    return eq_functor(elem,hash, key ...);
};

template<class T,class H,class E,class TV,class A>
bool hash_table<T,H,E,TV,A>::eval_eq_functor(details::check_assign_false, 
                                const value_type* elem, size_t hash, 
...
template<class T,class H,class E,class TV,class A>
void hash_table<T,H,E,TV,A>::remove_delete_marks(value_type** pos,
                              value_type** first_deleted) const
{
...
{
    if (m_capacity.value() >= n_elements*4 
            && m_capacity.value() > ht_capacity::min_size)
    {
        change_size(false);
    }
};
...
template<class T,class H,class E,class TV,class A>
inline size_t hash_table<T,H,E,TV,A>::capacity() const
{ 
    return m_capacity.value(); 
};

template<class T,class H,class E,class TV,class A>
inline const typename hash_table<T,H,E,TV,A>::value_type* const* 
hash_table<T,H,E,TV,A>::get_entries() const
{
    return entries; 
};
