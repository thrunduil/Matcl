...
template<class T>
struct MATCL_SF_EXPORT owens_t_helper
{
    using return_type   = typename md::unify_types<T,Float>::type;

    static return_type  eval_owens_t(const T& h, const T& a);
};
...
matcl::owens_t(const S1& h, const S2& a)
{
    using S = typename md::unify_types2_promote<S1,S2,Float>::type;
    return details::owens_t_helper<S>::eval_owens_t(S(h),S(a));
};
