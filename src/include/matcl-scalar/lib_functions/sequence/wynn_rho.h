...
// at points x0 < x1 < ... < xn for n -> infinity by means of the rho 
// algorithm of P. Wynn. An estimate of the absolute error is also given.
// The rho algorithm is exact for a sequence of the form:
//      s_n = (sxn^k + a1 xn^{k-1} + ... + ak)/(xn^k + b1 xn^{k-1} + ... + bk)
//            xn -> infinity
// The rho algorithm is able to accelerate converge of logarithmically
// convergent sequences, but is not able to accelerate linear convergence 
// nor find an antilimit of divergenent sequences
//
...
class wynn_rho : protected details::seq_transform_base<Float>
{
    private:        
    ...
    public:
        ...
        wynn_rho(int precision = 0);

        // estimate the limit of a given sequence;
        // x        - a new point greater than all previous points
...
#include "matcl-scalar/details/sequence/wynn_rho.inl"