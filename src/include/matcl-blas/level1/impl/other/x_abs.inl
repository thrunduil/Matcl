...
struct x_abs<TY, TX, Rows, false, details::true_t>
{
    force_inline
    ...
    static void eval(TY* Y1, Integer Y_step, const TX* X1, Integer X_step, Integer rows)
    {
		...
		for (Integer i = 0; i < size; ++i)
			Y[i*Y_step] = TY(abs(X[i*X_step]));
    };
};

template<class TY, Integer Rows>
struct x_abs<TY, TY, Rows, true, details::true_t>
{
    force_inline
    ...
    static void eval(TY* Y1, Integer Y_step, const TY* X1, Integer X_step, Integer rows)
    {
        ...
        return x_abs<TY,TY,Rows,false,details::true_t>::eval(Y1, Y_step, X1, X_step, rows);
    };
};
