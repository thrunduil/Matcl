...
struct axpy<TY, TX, TA, Rows, false, details::true_t>
{
    force_inline
...
struct axpy<TY, TY, TA, Rows, true, details::true_t>
{
    force_inline
    static void eval(TY* Y1, const TY* X1, Integer rows, const TA& a)
    {
		...
		for (Integer i = 0; i < size_1; ++i)
        {
			...
			simd_y res_1 = fma_f(sa,sX_1,sY_1);
    ...
    static void eval(TY* Y1, Integer Y_step, const TY* X1, Integer X_step, Integer rows, const TA& a)
    {
        ...
        return axpy<TY,TY,TA,Rows,false,details::true_t>::eval(Y1, Y_step, X1, X_step, rows, a);
    };
};
