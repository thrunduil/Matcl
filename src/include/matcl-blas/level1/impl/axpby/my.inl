...
struct my<TY, Rows, false, details::true_t>
{
    force_inline
    static void eval(TY* Y1, Integer rows)
    {
    ...
    force_inline
    static void eval(TY* Y1, Integer Y_step, Integer rows)
    {
        if (Y_step == 1)
            return eval(Y1,rows);
		...
		for (Integer i = 0; i < size; ++i)
			Y[i*Y_step] = -Y[i*Y_step];
    };
};

template<class TY, Integer Rows>
struct my<TY, Rows, true, details::true_t>
{
    force_inline
    static void eval(TY* Y1, Integer rows)
    {
    ...
    static void eval(TY* Y1, Integer Y_step, Integer rows)
    {
        ...
        return my<TY, Rows, false, details::true_t>::eval(Y1, Y_step, rows);
    };
};
