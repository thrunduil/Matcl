...
{
    private:
        using impl_type         = details::scotch_graph_impl;
        using impl_ptr          = std::shared_ptr<impl_type>;

        friend details::scotch_graph_impl;
    ...
    public:
        ...
        // prepare partitioner for an undirected graph represented by a symmetric 
        // matrix A; if use_weights is true, then values in the matrix will be 
        // used as weights, otherwise unit edge weights are assumed; see also 
        // set_node_weights
...
// wrapper class over scotch's mesh
class MATCL_LINALG_EXPORT scotch_mesh
{
    private:
        using impl_type         = details::scotch_mesh_impl;
        using impl_ptr          = std::shared_ptr<impl_type>;

        friend details::scotch_mesh_impl;
    ...
    public:
        ...
        // prepare partitioner for a bipartite graph represented by a general matrix A;
        scotch_mesh(const Matrix& A);
