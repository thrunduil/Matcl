...
MATCL_REGISTER_SCALAR_FUNC(f_##fn, fn, mp_rational, mdyf::fn)
...
MATCL_REGISTER_SCALAR_FUNC(uminus, operator-, mp_rational, mdyf::op_uminus)
MATCL_REGISTER_SCALAR_FUNC(op_not, !, mp_rational, mdyf::op_not)
MATCL_REGISTER_SCALAR_FUNC(op_bool, (bool), mp_rational, mdyf::op_bool)

MATCL_REGISTER_BIN_FUNC(ldexp1, matcl::ldexp, mp_rational, Integer, mdyf::ldexp)
MATCL_REGISTER_BIN_FUNC(frexp1, matcl::frexp, mp_rational, Integer&, mdyf::frexp)
MATCL_REGISTER_BIN_FUNC(modf1, matcl::modf, mp_rational, mp_float&, mdyf::modf)
MATCL_REGISTER_BIN_FUNC(f_mod, matcl::mod, mp_rational, mp_rational, mdyf::mod)
MATCL_REGISTER_BIN_FUNC(f_rem, matcl::rem, mp_rational, mp_rational, mdyf::rem)
MATCL_REGISTER_BIN_FUNC(f_dim, matcl::fdim, mp_rational, mp_rational, mdyf::fdim)

MATCL_REGISTER_TEMPL_1_FUNC(f_factorial,matcl::factorial,mp_rational,Integer,mdyf::factorial)
MATCL_REGISTER_TEMPL_1_FUNC(f_double_factorial,double_factorial,mp_rational,Integer,mdyf::double_factorial)
MATCL_REGISTER_TEMPL_2_FUNC(f_binomial_coefficient,binomial_coefficient,mp_rational,Integer,Integer,
...
//--------------------------------------------------------------
//promotion
struct conv_mpint_big : mdy::register_convert<conv_mpint_big, mdy::convert_promotion>
{
	static MP_rational eval(const MP_int& val) { return MP_rational(val.get()); };
};

//equivalent
struct conv_int_big : mdy::register_convert<conv_int_big, mdy::convert_equivalent>
{
	static MP_rational eval(const OInteger& val) { return MP_rational(val.get()); };
};

//decay

//explicit
struct conv_float_mprat : mdy::register_convert<conv_float_mprat, mdy::convert_explicit>
{
	static MP_rational eval(const OFloat& val) { return MP_rational(val.get()); };
};
struct conv_real_mprat : mdy::register_convert<conv_real_mprat, mdy::convert_explicit>
{
	static MP_rational eval(const OReal& val) { return MP_rational(val.get()); };
};

struct conv_mpreal_mprat : mdy::register_convert<conv_mpreal_mprat, mdy::convert_explicit>
{
	static MP_rational eval(const MP_float& val) { return MP_rational(val.get()); };
};
struct conv_mprat_real : mdy::register_convert<conv_mprat_real, mdy::convert_explicit>
{
	static OReal eval(const MP_rational& val) { return OReal(val.get().cast_float()); };
};

//cast
struct conv_mprat_int : mdy::register_convert<conv_mprat_int, mdy::convert_cast>
{
	static OInteger eval(const MP_rational& val) { return OInteger(val.get().cast_int()); };
};

struct conv_compl_mprat : mdy::register_convert<conv_compl_mprat, mdy::convert_cast>
{
	static MP_rational eval(const OComplex& val) 
    { 
        return MP_rational(convert_scalar<mp_rational>(val.get())); 
    };
};
struct conv_fcompl_mprat : mdy::register_convert<conv_fcompl_mprat, mdy::convert_cast>
{
	static MP_rational eval(const OFloat_complex& val) 
    { 
        return MP_rational(convert_scalar<mp_rational>(val.get())); 
    };
};
struct conv_mpcompl_mprat : mdy::register_convert<conv_mpcompl_mprat, mdy::convert_cast>
{
	static MP_rational eval(const MP_complex& val) 
    { 
        return MP_rational(convert_scalar<mp_rational>(val.get())); 
    };
};
